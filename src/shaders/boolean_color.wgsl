...
const OKLCH_PALETTES = array(
    ...
    OklchGradient(vec3f(0.4, 0.1, radians(40)), vec3f(0.7, 0.1, radians(40)))
);
...
fn fragment_main(input: Interpolated) -> @location(0) vec4f {
    ...
    let b_swatches = palette_lookup(OKLCH_PALETTES[1], b_step);
    ...
    // background layer
    var color = vec3f(0.0);
    color = mix(color, vec3f(0.0, 1.0, 0.0), mask_a);
    color = mix(color, vec3f(1.0, 0.0, 0.0), mask_b);
