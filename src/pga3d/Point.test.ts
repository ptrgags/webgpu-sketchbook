...
describe('Point', () => {
  it('converts to direction', () => {
    const a = new Point(2, -5, 1)

    const result = a.to_direction()

    const expected = new Direction(2, -5, 1)
  ...
  it('adding a direction returns the correct point', () => {
    ...
    const expected = new Point(4, 6, 3)
    expect(result).toBePoint(expected)
  })
  ...
  it('dist_sqr returns euclidean distance squared between two points', () => {
    const a = new Point(-1, 2, 6)
    const b = new Point(3, -4, 2)
  ...
  it('dist returns euclidean distance between two points', () => {
    ...
    // sqrt((3 + 1)^2 + (-4 - 2)^2 + (2 - 6)^2)
    // sqrt(4^2 + 6^2 + 4^2) = sqrt(68)
    const expected = Math.sqrt(68)
    expect(result).toBe(expected)
  })
})
