...
fn fragment_main(input: Interpolated) -> @location(0) vec4f {
    ...
    let keyboard_mask = rect_mask(input.uv, vec2f(0.0, 1.0 / 3.0), vec2f(1.0, 1.0 / 3.0));

    let thick_key_mask = interval_mask(u_white_key, WHITE_KEY_MARGIN, WHITE_KEY_WIDTH);
    let white_key_mask = thick_key_mask * keyboard_mask;

    let black_key_section = rect_mask(input.uv, vec2f(0.0, 0.45), vec2f(1.0, 1.0));
    let narrow_key_mask = interval_mask(u_black_key, BLACK_KEY_MARGIN, BLACK_KEY_WIDTH);
    let is_valid = f32(semitones_black != DUMMY);
    let black_key_mask = narrow_key_mask * black_key_section * keyboard_mask * is_valid;
